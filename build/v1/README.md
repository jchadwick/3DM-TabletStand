...
- `tablet_stand_v1_usb_detail.png`: rear view of the hidden pigtail, open braided-cable clips, and sleeve channel.
- `tablet_stand_v1_multiview.png`: combined design-review preview.

## Validation

The preview PNGs are rendered directly from in-memory CadQuery tessellation by `scripts/render_cadquery_preview.py` using Trimesh's depth-buffered renderer; they do not reread the exported STL. Trimesh validation reports one watertight connected component for each printable STL. The kiosk-oriented main envelope is 216.0 x 70.52 x 130.49 mm; the end-stop local envelope remains 10.2 x 130.0 x 19.2 mm.

## Before printing

This first pass is intended for design review. Confirm button/speaker/camera clearances, the meaning of the 9.6 mm connector annotation, and M3 screw length. Test the 4.15 mm cable channel with a short coupon, test-slice the 216 mm main-body span against the printer's usable bed—not merely its nominal bed size—and decide whether the integrated tilted sleeve needs support or should become a separately printed bracket in version 2.
