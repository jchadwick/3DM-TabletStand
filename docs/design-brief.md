...
The first parametric concept, now archived at Git commit `560c6e8`, used a 1.0 mm total allowance in X and Y and 0.8 mm in Z, narrow U-shaped rails on the near and far long edges, internal right corner stops behind a closed USB-C plug housing, and a full-height removable left end stop with an M3 clearance hole. Its measurements that survived physical testing were promoted into the active V2 core; the superseded model itself is not maintained.
