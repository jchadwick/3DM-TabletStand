...
| Exposed finish | Front-frame opening follows the supplied tablet's approximately 7.0 mm plan corner radius; outer frame radius is 17.0 mm for a uniform 10 mm frame wall; rails/walls use 1.45 mm plan corners and 0.70 mm edge fillets; lips use 1.05 mm plan corners and 0.60 mm edge fillets | Rounded to match the tablet silhouette and soften the visible holder |
...
- The left wing retains the tested button channel and adds a continuous rounded left wall and full-width screen-facing cap. The right wing retains the tested USB-C housing and open 16 × 8 mm cable rectangle. All V2 cap-specific tapered plugs, sockets, and receiver bulges are absent from V3.
...
- Exposed perimeter, rail, lip, and integral-left-wall edges use larger plan-corner radii and longitudinal fillets so the holder feels softer than the earlier sharp-edged print. The front-frame opening follows the supplied tablet mesh's approximately 7 mm corner curve, and the outer frame radius is 17 mm so the 10 mm frame wall remains uniform. V3 has no vestigial screw-era projection below the left corner.
...
- Previous full-cradle, end-stop, combined V3 coupon, and pre-revision right-wing G-code are stale. Do not print them. The tongue/receiver fit and wedge retention gates passed. The right-wing production job completed with support-removal difficulty noted, but the active rounded-finish geometry requires a new slice; do not start the left wing or other parts until their fresh slices and support/adhesion review are complete.
