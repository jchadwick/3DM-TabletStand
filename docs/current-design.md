...
The project is a simple, sleek, FDM-printable landscape holder for a 2024 onn. 8-inch tablet. It mounts permanently to the accessible top end of a vertical 32 mm OD tube. V3 is the active split-cradle candidate: its physical tongue/receiver coupon fit despite severe print defects, and the corrected wedge retained and released successfully. The corrected full-span right wing was also physically tested: despite severe stringing, the tablet slid in smoothly and fit snugly with very good clearance. V3 preserves that fit plus the tested button, USB-C, tube, and cable interfaces while replacing both the 215 mm-wide cradle print and separate left cap with two bed-friendly wings. Superseded V1 is archived only in Git history at commit `560c6e8` and is not maintained.
...
- The first `tablet_stand_v3_lock_coupon_all3.stl` print physically passed tongue/receiver fit despite severe adhesion and stringing. The corrected pin then retained and released successfully. Do not rescale or change the confirmed tongue/socket clearances. The stale combined coupon G-code and all pre-finish right-wing slices have been removed from the active tree and remain recoverable from Git history. The prior right-wing production job used snug 45° supports and a 5 mm brim and completed on 2026-08-10, but the user found the support material difficult to distinguish/remove in this PLA; any fresh slice needs a more removable support strategy. The left wing and remaining parts still require their own review and authorization.
...
- Previous full-cradle, end-stop, combined V3 coupon, and all right-wing G-code files are stale for the current geometry and have been removed from the active tree. Do not restore or print them; Git history retains them as process evidence. The tongue/receiver fit and wedge retention gates passed. The corrected full-span right-wing job completed from prior geometry using rear-face-down, 0.16 mm layers, 3 walls, 20% grid infill, 55° snug build-plate-only supports, a 0.24 mm Z gap, 0.70 mm XY gap, 3 interface layers, and a 5 mm brim. The result was very stringy, but the tablet slid in very smoothly and fit snugly with very good clearance. Preserve that cavity fit. The later 5.3 mm screen corners, 16 × 12 mm USB opening, 1.30/0.90 mm finish, single smooth front ring, closed left rail lead, and full-travel button groove are CAD-validated but not physically tested. A fresh slice and physical confirmation are required before another right-wing print. Do not start the left wing or other parts without separate authorization.
