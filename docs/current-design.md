...
| Rear cable opening | 16 × 8 mm (Y × X), centered across the tablet edge: 4 mm inboard + 4 mm outboard | Photo-requested 4 mm inboard shift restores the unused outboard half of the rear floor and cuts the same green-marked area inboard; the plug chamber above remains 8 mm deep |
...
- The right side presents a continuous enclosed screen-facing face: the outer wall and full-depth cap span the complete 139 mm V3 perimeter, including the extended top/bottom corners. The plug chamber remains open toward the tablet cavity and keeps its full 8 mm depth above the rear floor for the recorded 6.50 mm projection. The 16 mm-wide × 8 mm-deep rear-floor rectangle is shifted 4 mm inboard: the former outboard half is restored as floor and the same 4 mm is cut from the green-marked tablet-side floor. This retains the cable turn area while removing the visibly unused outboard floor opening.
...
- Exposed perimeter, rail, lip, and integral-left-wall edges use the existing plan-corner radii and longitudinal fillets. The rear frame opening follows the supplied tablet mesh's approximately 7 mm corner curve, and the outer frame radius remains 28 mm. At the front lip plane, the four corner infills form a 196.6 × 119.6 mm rounded screen opening: its 5.3 mm radius is concentric with the tablet's approximately 7 mm radius and leaves only the already proven 1.7 mm straight-edge overlap. The infills hide the tablet glass/body corners while leaving the insertion cavity below them unchanged. Continuous top/bottom edge rails absorb the joiner blocks on the rear so the screen-facing perimeter reads as a straight, uninterrupted line. V3 has no vestigial screw-era projection below the left corner.
...
- Before another full right wing, print or otherwise physically verify the revised right-side USB region rear-face down in the intended PLA process. Slide the tablet's right edge through the production rails, seat it against the internal stops, connect the real USB-C adapter, and confirm that the complete thick cable section turns through the relocated 16 × 8 mm rear rectangle without pinching or forcing the tablet. The prior physical wing proves tablet fit, not this relocated floor opening.
