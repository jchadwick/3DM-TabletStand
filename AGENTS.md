...
| Outside finish | Front-frame opening follows the supplied tablet's approximately **7.0 mm** plan corner radius, with a pronounced **28.0 mm** outer frame radius; continuous top/bottom edge rails shroud the joiners on the rear while keeping the screen-facing perimeter straight; the integral left outer wall and cap span the full **139 mm** V3 envelope, with any entry relief confined to the rear/inner path; rails/walls use 1.45 mm plan corners and 0.70 mm edge fillets, lips use 1.05 mm plan corners and 0.60 mm edge fillets, and the integral left closure retains a 1.70 mm plan corner and 0.85 mm edge fillet |
...
| Current status | The physical coupon passed: the production tongue and receiver fit, and the corrected wedge/pin retained and released successfully. Retain the 0.50 mm root / 1.10 mm lead clearances. The original support-free wedge G-code is stale; the corrected pin strategy is now accepted. The pre-revision right-wing production print completed on 2026-08-10, but its supports were difficult to distinguish/remove in the user's PLA; the new rounded-finish geometry requires a fresh slice. The left wing and remaining parts are not yet authorized |
