...
| Tablet retention | Tablet slides in from the left through narrow long-edge rails; a separate screw-free left cap then slides horizontally in from the left on two enlarged tapered rail-end plugs. The plugs enter reinforced, open-ended sockets in the top and bottom rail ends and seat against the socket backs. The revised PLA fit uses 0.60 mm total root clearance and 0.80–1.00 mm total tip clearance; there is no screw, hook, detent bump, projecting lower nub, or support-trapping dovetail; adhesive is optional |
...
| Current status | V2 is the sole active model. The failed support-trapping dovetail is superseded by two horizontally inserted tapered rail plugs with support-free 45-degree socket roofs. The first twin-plug PLA coupon was too tight and the small plug broke during fitting; the parametric revision enlarges both the socket and plug and targets 0.60 mm total root clearance. Revised geometry, watertight exports, previews, and a replacement coupon are required before the full cradle and cap are re-sliced; existing full-part G-code is stale |
