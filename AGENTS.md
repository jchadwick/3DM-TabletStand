...
| Tablet retention | In V3, seat the tablet's USB-C/right edge in the right cradle wing, then bring the fully enclosed left wing onto the tablet and join the stepped center seam. The left edge is one continuous rounded wall integral to the wing; there is no separate cap, plug, socket, screw, hook, detent, or nub. Adhesive assembly captures the tablet, so do not bond until serviceability is accepted |
...
| V2 print layouts | Cradle rear face down; bracket foot down; sleeve flange down with tube bore open upward; left cap broad outside wall face down so both tapered plugs build vertically |
| V3 cradle split | Current visual-review candidate replaces the wide cradle and separate cap with a fully enclosed **114.83 × 130.00 × 14.20 mm left wing** and **119.33 × 130.00 × 14.20 mm right wing**. Both print rear-face-down |
| V3 cradle joint | **0.35 mm-clearance stepped planar seam**, X = −8 mm at the rails and X = +8 mm through the center, plus three recessed **44 × 6 × 1.6 mm** loose adhesive keys. The existing 74 × 36 mm rear bracket also bridges the center seam; no undercut or generated support |
...
| Current status | The corrected V3 split-cradle candidate is built, watertight, validated, rendered, and visually inspected with a continuous integral left wall and no separate end cap. It preserves the tested button, USB-C, tablet, tube, and cable interfaces while avoiding the near-bed-width print. No V3 part has been sliced or authorized for printing, and all earlier full-cradle G-code is stale |
...
- `cad/tablet_stand_v3.py`: current split-cradle candidate, splice keys, V3 assembly, print orientations, and exports.
...
- `scripts/validate_model_v3.py`: active V3 split, splice-key, fit-preservation, print-layout, and artifact checks.
- `build/v3/`: current V3 review STEP, six printable STL exports, parameters, and multi-view previews. The structural alignment-key STL is printed twice and the cradle-splice-key STL three times.
...
Before calling any version final or print-ready, obtain or verify: cable/connector width; button, speaker, camera, and microphone clearances; available tube length; printer, nozzle, filament, build volume, intended print orientation, and adhesive/surface preparation. Print small rail, sleeve, cable-channel, alignment-key, and screw-free twin-plug fit coupons before committing to the full holder.
