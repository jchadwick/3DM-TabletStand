...
| V2 print layouts | Cradle rear face down; bracket foot down; sleeve flange down with tube bore open upward; slide stop broad outside wall face down so the dovetail builds vertically |
...
| Current status | V2 is the sole active model. The M3 stop and hole are removed; the clean closed-bottom dovetail stop, nub-free rounded lower-left corner, softened edges, and exact two-piece fit coupon are geometrically validated. The first support-free left-slide coupon failed from loose/collapsed extrusion and stringing; the revised 28 mm-gap, grid-supported coupon retry is printing and must be physically verified before the full cradle |
