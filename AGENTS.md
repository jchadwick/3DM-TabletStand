...
| Tablet retention | Tablet slides in from the left through narrow long-edge rails; a separate screw-free left cap then slides horizontally in from the left on two enlarged tapered rail-end plugs. The plugs enter reinforced, open-ended sockets in the top and bottom rail ends and seat against the socket backs. The revised PLA fit targets 0.45 mm total root clearance and 0.85 mm total tip clearance; there is no screw, hook, detent bump, projecting lower nub, or support-trapping dovetail; adhesive is optional |
...
| Current status | V2 is the sole active model. The failed support-trapping dovetail is superseded by two horizontally inserted tapered rail plugs with support-free 45-degree socket roofs. The enlarged socket coupon fit well but slightly loose; the socket is retained and the parametric plug is tightened to 0.45 mm total root clearance while staying thicker than the broken first version. Revised watertight exports, previews, and a plug-only coupon are required before the full cradle and cap are re-sliced; existing full-part G-code is stale |
...
- `build/v2/`: active generated STEP, five production STL files, right-side, button, and two-piece twin-plug fit-coupon STLs, parameters, previews, and separate support-free coupon G-code jobs. The alignment-key STL is printed twice.
