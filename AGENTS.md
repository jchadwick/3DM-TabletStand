...
| Tablet retention | Tablet slides in from the left through narrow long-edge rails; a separate screw-free left cap then slides horizontally in from the left on two tapered rail-end plugs. The plugs enter reinforced, open-ended sockets in the top and bottom rail ends and seat against the socket backs. There is no screw, hook, detent bump, projecting lower nub, or support-trapping dovetail; adhesive is optional |
...
| Current status | V2 is the sole active model. The failed support-trapping dovetail is superseded by two horizontally inserted tapered rail plugs with support-free 45-degree socket roofs. Geometry, watertight exports, horizontal insertion travel, previews, and two separate support-free coupon slices are validated. The physical twin-plug coupon must pass before the full cradle and cap are re-sliced; existing full-part G-code is stale |
